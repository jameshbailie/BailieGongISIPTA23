...
p <- p + theme_bw() + labs(y = NULL, x = NULL, color = NULL)+#, 
                           #caption = TeX(r"($\epsilon = 1)"),
                           #title = TeX(r"(Density of the Laplace mechanism, when $d(x,x') = 1$)")) + 
...
ggsave('posterBailie23Figs/LaplaceEx.pdf', width = 5, height = 5/0.99*0.47*6/7)
